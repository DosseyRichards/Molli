		...
		callback();
			...
			if(!err){
				if(req.decoded){
					...
					this_montage.save();
					this_montage.belongs_to_user = req.decoded._id;
					this_montage.belongs_to_username = req.decoded.username;					
...
exports.getUserMontages = function(req, res, next) {
	montage.find({belongs_to_user:req.decoded._id}).sort({created_on:-1}).limit(10).exec(function(err, docs){
		res.json({results:docs});
	});		
};
...
exports.getMoreUserMontage = function(req, res, next) {
	var skip_int = req.params.page * 10;
	montage.find({belongs_to_user: req.decoded._id}).skip(skip_int).sort({created_on:-1}).limit(10).exec(function(err, docs){
		res.json({results:docs});
		console.log(docs);
	});		
};
